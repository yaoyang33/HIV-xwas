...
while (TRUE) {
  ...
  if (auth_post$all_headers[[2]]$status == 302) {
    message("Successfully log in to DHS!")
    break
  } else {
    message("Incorrect email address and/or password. Please try again.")
  }
}
...
message(paste0("Log in with project: ", your.project))
...
for ( j in seq( length( country.numbers ) ) ){
	...
	message(paste0("Fetching data for country: ", this.name, "..."))
	...
	for ( this.link in link.urls ){
		...
		archive.links.all <- unlist( data.link[ select.datatype ] )
		...
		url_archive_zips <- gsub( "(.*)Filename=(.*)\\.(ZIP|zip)(.*)" , "\\2.zip" , archive.links.all)
		archive.links.toadd <- archive.links.all[!url_archive_zips %in% this_archive_zips]
		...
		url_live_zips <- gsub( "(.*)Filename=(.*)\\.(ZIP|zip)(.*)" , "\\2.zip" , live.links.all)
		live.links.toadd <- live.links.all[!url_live_zips %in% this_live_zips]
		
		# check whether the new version exists in url, if so, remove old files
		toadd_live_zips <- gsub( "(.*)Filename=(.*)\\.(ZIP|zip)(.*)" , "\\2.zip" , live.links.toadd)
		url_zips_check <- gsub("(\\d)(.)" , "\\1" , toadd_live_zips)
		this_zips_check <- gsub("(\\d)(.)" , "\\1" , this_live_zips)
    ...
    # DOWNLOAD ARCHIEVE FOLDER
		...
		for( file.url in live.links.toadd ){
		  
		  # maintain the zipped filename
		  zipname <- gsub( "(.*)Filename=(.*)\\.(ZIP|zip)(.*)" , "\\2.zip" , file.url )
		  zfn <- paste0( this_dir_live , "/" ,  zipname)
		  message(paste0("[Live]Downloading ", zipname, " in ", this.title, "..."))
