...
for ( j in seq( length( country.numbers ) ) ){
	...
	# pull all links
	link.urls <- xpathSApply( htmlParse( content( z ) ) , "//a" , xmlGetAttr , "href" )
	...
	for ( this.link in link.urls ){
		...
		all.links <- xpathSApply( htmlParse( content( z ) ) , "//div//a" , xmlGetAttr , "href" )
		...
		# create directory path for the survey
		dir.create( paste0( dir.archive, this.name , "/" , this.title ) )
		this_dir_archive <- paste0( dir.archive , this.name , "/" , this.title )
		if (length(live.links) != 0) {
		  dir.create( paste0( dir.live, this.name , "/" , this.title ) )
